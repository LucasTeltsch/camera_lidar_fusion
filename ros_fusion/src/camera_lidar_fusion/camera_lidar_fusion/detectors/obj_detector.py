...
from camera_lidar_fusion.detectors.img_obj_detector import ImageObjectDetector
from camera_lidar_fusion.detectors.pc_obj_detector import PointCloudObjectDetector
import time
...
class ObjectDetector:
    ...
    def __init__(self, img_model: str, pc_model: str):
        """
        ...
        self.img_obj_detector = ImageObjectDetector(img_model)
        self.pc_obj_detector = PointCloudObjectDetector(pc_model)

    def detect_objects(self, logger, sync_data_queue: Queue, post_process_queue: Queue):
        """
        ...
        while True:
            ...
            img_results = self.img_obj_detector.detect_objects(image)
            pc_results = self.pc_obj_detector.detect_objects(pc)
            
            logger.info(f"[ObjectDetector] FPS for camera and lidar object detection: {1 / (time.time() - start_time):.2f}")
            
            logger.info(f"pc_results: {pc_results}")

            sync_data_queue.task_done()

            post_process_queue.put((image, img_results, pc_results))
